...
_vue2.default.component('contact-list', {
	props: ['child'],
	...
	methods: {
		findChild: function findChild(arr, id) {
			var self = this;
		...
		deleteItems: function deleteItems(item) {
			var self = this;
			var index = this.$root.$data.arr.indexOf(item);

			if (typeof item.parentID === 'undefined' || item.parentID === 0) {
				var result = self.list(self.arr, item.sub);

				var removed = [];
				self.$root.$data.arr.map(function (el, idx) {
...
var contact = new _data2.default({
	el: '#list',
	data: {},
	created: function created() {
		var self = this;
	...
	methods: {
		readed: function readed(data) {
			var result = data.filter(function (item) {
				return !item.readed;
			});
			return result;
		},
		setRead: function setRead(id) {
			var self = this;

			var result = self.arr.map(function (item) {
		...
		findParent: function findParent() {
			var self = this;
			var parent = self.arr.filter(function (item) {
				if (typeof item.parentID === 'undefined' || item.parentID === 0) {
					self.setRead(item.ID);
